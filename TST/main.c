...
#include <png.h>
#include <pngconf.h>
PSP_MODULE_INFO("J aime", PSP_MODULE_USER, 2, 0);
...
int writeImage(char* filename, int width, int height, float *buffer, char* title)
{
    ...
    pspDebugScreenPrintf("here2");
	// Allocate memory for one row (3 bytes per pixel - RGB)
	row = (png_bytep) ((sceGeEdramGetAddr()) + (272 * 512 * 3));
...
{
	float *buffer = (float*)(sceGeEdramGetAddr());
	if (buffer == NULL) {
		pspDebugScreenPrintf("no malloc");
		
	}
    pspDebugScreenPrintf("malloc");
...
int main()
{
	...
	if (buffer == NULL) {
		pspDebugScreenPrintf("null");
	}
    pspDebugScreenPrintf("heeeere");
	// Save the image to a PNG file
	// The 'title' string is stored as part of the PNG file
	
	int result = writeImage("sa.png", width, height, buffer, "This is my test image");
    pspDebugScreenPrintf("okokkokkkk");
